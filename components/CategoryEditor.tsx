...
export function CategoryEditor({ categories, updateCategory }) {
  ...
  const handleAddNominee = () => {
    setEditingCategory({
      ...editingCategory,
      nominees: [...editingCategory.nominees, { id: Date.now().toString(), name: "" }],
    })
  }

  const handleRemoveNominee = (index) => {
    const updatedNominees = editingCategory.nominees.filter((_, i) => i !== index)
    setEditingCategory({ ...editingCategory, nominees: updatedNominees })
  }
              ...
              <div className="space-y-4">
                ...
                {editingCategory.nominees.map((nominee, index) => (
                  <div key={nominee.id} className="space-y-2">
                    <Label htmlFor={`nominee-name-${nominee.id}`}>Nominee Name</Label>
                    <div className="flex items-center space-x-2">
                      ...
                      />
                      <Button variant="destructive" onClick={() => handleRemoveNominee(index)}>
                        Remove
                      </Button>
                    </div>
                  </div>
